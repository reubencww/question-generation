...
def pipeline(
    ...
    **kwargs,
    ...
    if isinstance(tokenizer, (str, tuple)):
        if isinstance(tokenizer, tuple):
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                tokenizer[0], **tokenizer[1], use_fast=False
            )
        else:
    ...
    if isinstance(model, str):
        model = AutoModelForSeq2SeqLM.from_pretrained(model, return_dict=False)
            ...
            if isinstance(ans_tokenizer, (str, tuple)):
                if isinstance(ans_tokenizer, tuple):
                    ...
                    ans_tokenizer = AutoTokenizer.from_pretrained(
                        ans_tokenizer[0], **ans_tokenizer[1], use_fast=False
                    )
                else:
                    ans_tokenizer = AutoTokenizer.from_pretrained(
                        ans_tokenizer, use_fast=False
                    )
